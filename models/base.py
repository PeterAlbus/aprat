...
class BaseLearner(object):
    ...
    def _evaluate(self, y_pred, y_true):
        ret = {}
        cls_num_list = np.array([500]*self._total_classes)
        if self.args["imbalance"]:
            cls_num_list = np.array(self.args["lt_list"][:self._total_classes])
        # if 'cifar' in self.args['dataset']:
        thres_m, thres_f = 100, 20
        grouped = accuracy(y_pred.T[0], y_true, self._known_classes, self.args["init_cls"], self.args["increment"], cls_num_list, thres_m, thres_f)
    ...
    def _construct_exemplar_unified(self, data_manager, m):
        ...
        for class_idx in range(self._known_classes):
            ...
            # todo: add here
            logging.info(f"  class{class_idx} with {class_data.shape[0]} when constructing exempalrs")
        ...
        lt_number = data_manager.longtaillist
        for class_idx in range(self._known_classes, self._total_classes):
            ...
            max_ = lt_number[class_idx]
            saved = min(max_, m)
            logging.info(f"saving {saved} exemplars from {max_} samples of class{class_idx}")

            # Select
            selected_exemplars = []
            exemplar_vectors = []
            for k in range(1, saved + 1):
                S = np.sum(
                    exemplar_vectors, axis=0
                )  # [feature_dim] sum of selected exemplars vectors
                mu_p = (vectors + S) / k  # [n, feature_dim] sum to all vectors
            ...
            selected_exemplars = np.array(selected_exemplars)
            exemplar_targets = np.full(saved, class_idx)
            self._data_memory = (
                np.concatenate((self._data_memory, selected_exemplars))
                if len(self._data_memory) != 0
                else selected_exemplars
